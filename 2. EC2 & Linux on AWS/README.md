...
## 🚀 Hands-On: Launch EC2 & Host Web Page

Complete walkthrough to launch an EC2 instance and deploy a simple web server.
...
6. Key Pair (login)
   Option A: Create new key pair
     - Name: my-webserver-key
     - Type: RSA
     - Format: .pem (Linux/Mac) or .ppk (Windows/PuTTY)
     - Download and save securely
   
   Option B: Select existing key pair
...
# SSH into instance
ssh -i my-webserver-key.pem ec2-user@54.123.45.67

# Alternative: Use public DNS
ssh -i my-webserver-key.pem ec2-user@ec2-54-123-45-67.compute-1.amazonaws.com
...
1. Convert .pem to .ppk
   - Open PuTTYgen
   - Load → Select my-webserver-key.pem
   - Save Private Key → Save as my-webserver-key.ppk
...
2. Connect with PuTTY
   ...
   - Category → Connection → SSH → Auth
     → Browse → Select my-webserver-key.ppk
   - Click "Open"

3. Accept host key warning (first connection)
   - Click "Yes"
...
| Error | Solution |
| :--- | :--- |
| **Permission denied (publickey)** | Check key file permissions: `chmod 400 key.pem` |
| **Connection timed out** | Check security group allows SSH from your IP |
| **Host key verification failed** | Remove old entry: `ssh-keygen -R 54.123.45.67` |
| **No route to host** | Wait 2-3 minutes after launch, verify public IP |

---

### Step 3: Install Web Server & Host Page
...
# Expected output:
# Installed:
#   httpd.x86_64 0:2.4.58-1.amzn2023
...
# ============================================
# STEP 4: Create Dynamic HTML Page
# ============================================
...
<body>
    <div class="container">
        ...
        <p style="text-align: center; font-size: 1.2em;">
            Your web server is successfully running on Amazon EC2
        </p>
        
        <div class="info">
            <strong>Instance ID:</strong> 
            <span>$(curl -s http://169.254.169.254/latest/meta-data/instance-id)</span>
        </div>
        
        <div class="info">
            <strong>Availability Zone:</strong> 
            <span>$(curl -s http://169.254.169.254/latest/meta-data/placement/availability-zone)</span>
        </div>
        
        <div class="info">
            <strong>Instance Type:</strong> 
            <span>$(curl -s http://169.254.169.254/latest/meta-data/instance-type)</span>
        </div>
        
        <div class="info">
            <strong>Public IP:</strong> 
            <span>$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4)</span>
        </div>
        
        <div class="info">
            <strong>Region:</strong> 
            <span>$(curl -s http://169.254.169.254/latest/meta-data/placement/region)</span>
        </div>
    </div>
</body>
</html>
...
# Expected output:
# -rw-r--r--. 1 root root 1.8K Jan 12 10:30 /var/www/html/index.html
```
...
# You should see HTML content with instance details

# ============================================
# Get Instance Information
# ============================================

# Get your public IP (if you forgot to note it)
